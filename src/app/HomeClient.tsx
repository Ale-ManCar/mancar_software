  ...
  return (
    <main className="overflow-hidden">
        ...
        <div className="container mx-auto px-4 py-14 md:py-20">
          <div className="grid min-w-0 items-center gap-10 lg:grid-cols-[1fr_0.9fr]">
            <div className="min-w-0">
              <p className="section-kicker"><Text>Software creado desde Ecuador</Text></p>
              <h1 className="mt-5 max-w-4xl break-words text-[2rem] font-extrabold leading-[1.08] text-gray-950 sm:text-5xl md:text-7xl"><Text>
                Webs y sistemas que ayudan a vender, ordenar y crecer.
              </Text></h1>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-gray-600"><Text>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link href="/contacto" onClick={() => trackConversion('cta_click', { label: 'Solicitar orientación inicial', location: 'hero' })} className="btn-primary w-full sm:w-auto"><Text>
                  Solicitar orientación inicial
                </Text></Link>
                <a href="#servicios" onClick={() => trackConversion('cta_click', { label: 'Ver soluciones', location: 'hero' })} className="btn-secondary w-full sm:w-auto"><Text>
                  Ver soluciones
                </Text></a>
              </div>
              <div className="mt-10 grid max-w-2xl grid-cols-2 gap-3 sm:grid-cols-4">
                {stats.map(([value, label]) => (
                  <div key={label} className="min-w-0 rounded-2xl border border-gray-100 bg-gray-50 p-4">
                    <p className="font-display text-3xl font-extrabold text-gray-950"><Text>{value}</Text></p>
                    <p className="mt-1 break-words text-xs font-semibold leading-5 text-gray-500"><Text>{label}</Text></p>
                  </div>
                ))}
              </div>
            </div>
            <div className="image-frame h-[360px] min-w-0 md:h-[520px]">
              ...
              <div className="absolute bottom-5 left-5 right-5 rounded-2xl bg-white/92 p-5 backdrop-blur">
                <p className="text-sm font-extrabold text-primary-700"><Text>Diagnóstico primero</Text></p>
                <p className="mt-2 text-xl font-bold text-gray-950"><Text>Antes de desarrollar, definimos qué necesita lograr tu negocio.</Text></p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="metodo" className="bg-gray-50 py-16 md:py-20">
        <div className="container mx-auto px-4">
          <div className="mx-auto max-w-3xl text-center">
            <p className="section-kicker mx-auto"><Text>Por qué importa</Text></p>
            <h2 className="section-title"><Text>Tecnología con objetivo comercial, no solo diseño bonito.</Text></h2>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {benefits.map(([title, text]) => (
              <article key={title} className="soft-card p-6">
                <h3 className="text-xl font-bold text-gray-950"><Text>{title}</Text></h3>
                <p className="mt-3 leading-7 text-gray-600"><Text>{text}</Text></p>
              </article>
            ))}
          </div>
          <div className="mt-10 rounded-[2rem] border border-gray-200 bg-white p-6 shadow-xl shadow-gray-950/5 md:p-8">
            <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-start">
              <div>
                <p className="section-kicker"><Text>Confianza</Text></p>
                <h3 className="mt-5 text-3xl font-extrabold leading-tight text-gray-950 md:text-4xl"><Text>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {trustProof.map((proof) => (
                  <article key={proof.title} className="rounded-2xl border border-gray-100 bg-gray-50 p-5">
                    <p className="font-display text-2xl font-extrabold text-primary-700"><Text>{proof.metric}</Text></p>
                    <h4 className="mt-3 text-lg font-bold text-gray-950"><Text>{proof.title}</Text></h4>
                    <p className="mt-2 text-sm leading-6 text-gray-600"><Text>{proof.text}</Text></p>
                  </article>
                ))}
              </div>
            </div>
      ...
      <section id="servicios" className="bg-white py-16 md:py-20">
        <div className="container mx-auto px-4">
          <div className="mx-auto max-w-3xl text-center">
            <p className="section-kicker mx-auto"><Text>Soluciones</Text></p>
            <h2 className="section-title"><Text>Construimos lo que tu negocio necesita para avanzar.</Text></h2>
            <p className="section-copy"><Text>
            ...
            {serviceDetails.map((service, index) => (
              <article key={service.title} className="soft-card overflow-hidden">
                <div className="grid h-full md:grid-cols-[0.86fr_1.14fr]">
                  ...
                  <div className="p-6">
                    <p className="text-sm font-extrabold text-primary-700"><Text>{service.label}</Text></p>
                    <h3 className="mt-2 text-2xl font-bold text-gray-950"><Text>{service.title}</Text></h3>
                    <p className="mt-3 text-sm leading-6 text-gray-600"><Text>{service.fullDescription}</Text></p>
                    <button
                      type="button"
                      onClick={() => {
      ...
      <section className="bg-gray-50 py-16 md:py-20">
        <div className="container mx-auto px-4">
          <div className="grid items-center gap-10 lg:grid-cols-[0.9fr_1fr]">
            ...
            <div>
              <p className="section-kicker"><Text>Método de trabajo</Text></p>
              <h2 className="section-title"><Text>Primero entendemos el negocio. Después elegimos la tecnología.</Text></h2>
              <p className="section-copy"><Text>
              ...
              <div className="mt-7 grid gap-4 sm:grid-cols-2">
                {steps.map(([number, title, text]) => (
                  <article key={number} className="rounded-2xl border border-gray-100 bg-white p-5">
                    <p className="font-display text-2xl font-extrabold text-primary-700"><Text>{number}</Text></p>
                    <h3 className="mt-2 text-lg font-bold text-gray-950"><Text>{title}</Text></h3>
                    <p className="mt-2 text-sm leading-6 text-gray-600"><Text>{text}</Text></p>
                  </article>
                ))}
              </div>
            </div>
      ...
      <section className="dark-section py-16 text-white md:py-20">
        <div className="container mx-auto px-4">
          <div className="mx-auto max-w-3xl text-center">
            <p className="section-kicker mx-auto border-white/10 bg-white/10 text-white"><Text>Inversión</Text></p>
            <h2 className="section-title text-white"><Text>Puntos de partida claros para cotizar.</Text></h2>
            <p className="section-copy text-gray-300"><Text>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {plans.map((plan, index) => (
              <article key={plan.name} className={`rounded-2xl border p-6 ${index === 1 ? 'border-primary-300 bg-white text-gray-950' : 'border-white/10 bg-white/8 text-white'}`}>
                {index === 1 && <p className="mb-4 inline-flex rounded-full bg-secondary-100 px-3 py-1 text-xs font-extrabold text-secondary-800"><Text>Más solicitado</Text></p>}
                <h3 className="text-xl font-bold"><Text>{plan.name}</Text></h3>
                <p className={`mt-4 font-display text-3xl font-extrabold ${index === 1 ? 'text-primary-700' : 'text-primary-200'}`}><Text>{plan.price}</Text></p>
      ...
      <section className="bg-white py-16 md:py-20">
        <div className="container mx-auto px-4">
          <div className="grid items-center gap-8 rounded-[2rem] border border-gray-100 bg-gray-50 p-6 md:grid-cols-[0.9fr_1.1fr] md:p-10">
            <div>
              <p className="section-kicker"><Text>Portafolio</Text></p>
              <h2 className="mt-5 text-3xl font-extrabold leading-tight text-gray-950 md:text-5xl"><Text>
                Soluciones reales para negocios que necesitan avanzar.
              </Text></h2>
              <p className="mt-5 leading-8 text-gray-600"><Text>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="soft-card p-5">
                <p className="font-display text-3xl font-extrabold text-gray-950"><Text>{successCases.length}</Text></p>
                <p className="mt-1 text-sm font-semibold text-gray-500"><Text>casos documentados</Text></p>
              </div>
              <div className="soft-card p-5">
                <p className="font-display text-3xl font-extrabold text-gray-950"><Text>2</Text></p>
                <p className="mt-1 text-sm font-semibold text-gray-500"><Text>demos verificadas</Text></p>
              </div>
              <div className="soft-card p-5">
                <p className="font-display text-3xl font-extrabold text-gray-950"><Text>100%</Text></p>
                <p className="mt-1 text-sm font-semibold text-gray-500"><Text>enfoque comercial</Text></p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="contacto" className="bg-gray-50 py-16 md:py-20">
        <div className="container mx-auto px-4">
          <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-center">
            <div>
              <p className="section-kicker"><Text>Contacto</Text></p>
              <h2 className="mt-5 max-w-xl text-4xl font-extrabold leading-tight text-gray-950 md:text-5xl"><Text>Cuéntanos qué quieres mejorar.</Text></h2>
              <p className="mt-5 max-w-2xl text-lg leading-8 text-gray-600"><Text>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                <a href="tel:+593986951419" onClick={() => trackConversion('phone_click', { location: 'home-contact' })} className="rounded-2xl border border-gray-200 bg-white p-4 transition hover:border-primary-200 hover:bg-primary-50">
                  ...
                  <span className="mt-1 block font-bold text-gray-950"><Text>+593 98 695 1419</Text></span>
                </a>
                <a href="mailto:mancarsoftwares@gmail.com" onClick={() => trackConversion('email_click', { location: 'home-contact' })} className="rounded-2xl border border-gray-200 bg-white p-4 transition hover:border-primary-200 hover:bg-primary-50">
                  ...
                  <span className="mt-1 block break-words font-bold text-gray-950"><Text>mancarsoftwares@gmail.com</Text></span>
                </a>
              </div>
            </div>
            <div className="rounded-2xl border border-gray-200 bg-white p-5 text-gray-900 shadow-xl shadow-gray-950/5 md:p-6 lg:p-7">
              <div className="flex flex-col gap-2 border-b border-gray-100 pb-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <h3 className="text-2xl font-bold leading-tight text-gray-950"><Text>Cuéntanos tu caso</Text></h3>
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-gray-600"><Text>
                    Recibiremos tu solicitud con el contexto necesario para responderte con una orientación clara.
                  </Text></p>
                </div>
                <span className="w-fit rounded-full bg-primary-50 px-3 py-1 text-xs font-extrabold text-primary-800"><Text>Respuesta directa</Text></span>
              </div>
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="service-dialog-title" className="max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="p-6">
              <div className="mb-4 flex items-center justify-between gap-4">
                <h3 id="service-dialog-title" className="text-2xl font-bold text-gray-950"><Text>{serviceDetails[selectedService].title}</Text></h3>
                <button type="button" aria-label={t("Cerrar servicio")} onClick={() => setSelectedService(null)} className="text-2xl text-gray-400 transition hover:text-gray-600"><Text>×</Text></button>
              </div>
              <p className="mb-6 text-gray-600"><Text>{serviceDetails[selectedService].fullDescription}</Text></p>
              <h4 className="mb-2 font-semibold text-gray-950"><Text>Características destacadas</Text></h4>
              <ul className="mb-6 list-disc space-y-1 ps-5 text-gray-600">{serviceDetails[selectedService].features.map((feature) => <li key={feature}><Text>{feature}</Text></li>)}</ul>
            </div>
          </div>
        </div>
      )}
